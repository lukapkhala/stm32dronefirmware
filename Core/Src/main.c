...
//
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		m1 = fminf(fmaxf(m1, 1025), ESC_HIGH);
		m2 = fminf(fmaxf(m2, 1025), ESC_HIGH);
		m3 = fminf(fmaxf(m3, 1025), ESC_HIGH);
		m4 = fminf(fmaxf(m4, 1025), ESC_HIGH);

		ESC_Write_us(&htim2, TIM_CHANNEL_1, (uint16_t) m1); // M1: Front Right
		ESC_Write_us(&htim2, TIM_CHANNEL_2, (uint16_t) m2); // M2: Rear Right
		ESC_Write_us(&htim2, TIM_CHANNEL_3, (uint16_t) m3); // M3: Rear Left
		ESC_Write_us(&htim2, TIM_CHANNEL_4, (uint16_t) m4); // M4: Front Left
